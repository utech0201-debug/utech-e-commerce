...
const sections = [
  { id:"buyers", icon:ShoppingBag, title:"For shoppers", intro:"Browse products from UTECH and approved independent sellers, compare listings, add eligible items to your cart and place orders.", steps:[
  ...
  { id:"sellers", icon:Store, title:"For sellers", intro:"UTECH supports independent sellers with their own store identity, product catalog, order management, earnings and verification workflow.", steps:[
  ...
  { id:"seller-rules", icon:ShieldCheck, title:"Seller safety & marketplace rules", intro:"UTECH is designed so seller access, product publication and marketplace operations are separated by approval and ownership controls.", steps:[
  ...
  { id:"orders", icon:PackageCheck, title:"Orders & fulfillment", intro:"Marketplace orders are split into seller-specific order items so each seller can manage their own part of a multi-seller purchase.", steps:[
  ...
  { id:"admin", icon:BadgeCheck, title:"Marketplace administration", intro:"Authorized UTECH administrators have operational controls that are intentionally separated from seller accounts.", steps:[
...
export default function MarketplaceGuidePage() {
  return <div className={styles.page}>
    <section className={styles.hero}><div className={styles.heroInner}>
      ...
      <p>A practical guide for shoppers, sellers and marketplace operators. Learn what UTECH can do, how the marketplace works and where to go when you need help.</p>
      <div className={styles.heroActions}><Link href="/shop" className={styles.primary}>Explore the marketplace <ArrowRight size={17}/></Link><a href="#how-it-works" className={styles.secondary}>How it works</a></div>
    </div></section>
    <main className={styles.content}>
      <aside className={styles.toc}><div className={styles.tocCard}><span className={styles.tocLabel}>ON THIS PAGE</span>{sections.map(s=><a key={s.id} href={`#${s.id}`}>{s.title}</a>)}<a href="#quick-help">Quick help</a></div></aside>
      <div className={styles.main}>
        <section id="how-it-works" className={styles.intro}><div className={styles.sectionIcon}><Search size={21}/></div><div><span className={styles.kicker}>HOW IT WORKS</span><h2>One marketplace, different roles.</h2><p>Customers discover and buy products. Sellers operate their own marketplace stores. Authorized UTECH administrators handle approvals, verification, moderation and marketplace operations.</p></div></section>
        {sections.map(section=>{const Icon=section.icon;return <section id={section.id} key={section.id} className={styles.section}><div className={styles.sectionHeader}><div className={styles.sectionIcon}><Icon size={21}/></div><div><span className={styles.kicker}>UTECH MARKETPLACE</span><h2>{section.title}</h2><p>{section.intro}</p></div></div><div className={styles.steps}>{section.steps.map(([title,body],index)=><article className={styles.step} key={title}><span className={styles.number}>{String(index+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{body}</p></div></article>)}</div></section>})}
        <section id="quick-help" className={styles.help}><div className={styles.sectionHeader}><div className={styles.sectionIcon}><HelpCircle size={21}/></div><div><span className={styles.kicker}>NEED HELP?</span><h2>Go straight to the right place.</h2><p>Use these shortcuts instead of searching through the whole site.</p></div></div><div className={styles.quickGrid}>{quickLinks.map(([href,title,body])=><Link href={href} className={styles.quickCard} key={href}><div><strong>{title}</strong><span>{body}</span></div><ArrowRight size={17}/></Link>)}</div></section>
        <section className={styles.security}><ShieldCheck size={22}/><div><strong>Security reminder</strong><p>UTECH support will never need your password, OTP, PIN or full card security details. Do not share them through marketplace forms or messages.</p></div></section>
      </div>
    </main>
  </div>;
}
