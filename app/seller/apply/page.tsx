...
export default function SellerApplyPage() {
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setError("");setNotice("");const trimmedName=storeName.trim();const slug=slugify(storeSlug||trimmedName);const trimmedDescription=description.trim();if(!trimmedName||!slug){setError("Enter a valid brand/store name.");return;}if(containsRestrictedMarketplaceContent(trimmedName,trimmedDescription)){setError(marketplacePolicyNotice);return;}setSubmitting(true);const {data:userData}=await supabase.auth.getUser();if(!userData.user){router.replace("/auth/login?next=/seller/apply");return;}const {error:insertError}=await supabase.from("sellers").insert({user_id:userData.user.id,store_name:trimmedName,store_slug:slug,description:trimmedDescription||null});if(insertError){setError(insertError.code==="23505"?"That store name or store URL is already in use.":insertError.code==="23514"?marketplacePolicyNotice:insertError.message);setSubmitting(false);return;}setNotice("Application submitted. UTECH will review your seller account.");setSubmitting(false);router.refresh();}
  ...
  return <section className="section auth-section"><div className="container auth-container"><div className="auth-card"><span className="eyebrow">SELL ON UTECH</span><h1>Open your seller store.</h1><p>Use your own brand name and sell lawful products. New seller accounts start in review before products can be published.</p><form className="auth-form" onSubmit={submit}>
    <label>Brand / store name<input value={storeName} onChange={e=>{setStoreName(e.target.value);if(!storeSlug)setStoreSlug(slugify(e.target.value));}} placeholder="Example Tech Hub" required disabled={Boolean(notice)}/></label>
    ...
    <label>Store description<textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Tell customers what your brand sells." rows={5} disabled={Boolean(notice)}/></label>
    <p className="seller-field-help">{marketplacePolicyNotice} Products must pass UTECH review before publication.</p>
