  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeader}><div><span className={styles.kicker}>EXPLORE</span><h2>Popular categories</h2></div><Link href="/shop">See all →</Link></div>
        <div className={styles.categoryTiles}>
          {categories.map((category) => <Link href={`/shop?category=${category.query}`} key={category.query} className={styles.categoryTile}><span>{category.icon}</span><strong>{category.label}</strong><small>Shop now →</small></Link>)}
        </div>
      </section>
      ...
      <section className={styles.deals}>
        <div className={styles.sectionHeader}><div><span className={styles.kicker}>HOT PICKS</span><h2>Deals worth checking out</h2></div><Link href="/shop">View all →</Link></div>
